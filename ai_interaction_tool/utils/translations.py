...
def get_translations():
    ...
    return {
        "en": {
            ...
            "auto_keepalive_checkbox": "Auto keep-alive (Prevent timeout)",
            "auto_keepalive_timer": "Auto submit in: {minutes}:{seconds}",
            "auto_keepalive_message": "🤖 [SYSTEM AUTO KEEP-ALIVE] - Automatic message from ai_interaction tool (NOT from user). Purpose: maintain chat channel, prevent 15-min timeout. ⚠️ AGENT MUST IMMEDIATELY call mcp_AI_interaction_ai_interaction_tool to maintain communication channel. This is a system requirement, not optional. This message will repeat until user returns.",
        ...
        "vi": {
            ...
            "auto_keepalive_checkbox": "Tự động duy trì (Tránh timeout)",
            "auto_keepalive_timer": "Tự động gửi sau: {minutes}:{seconds}",
            "auto_keepalive_message": "🤖 [SYSTEM AUTO KEEP-ALIVE] - Tin nhắn tự động từ ai_interaction tool (KHÔNG phải user). Mục đích: duy trì kênh chat tránh timeout 15 phút. ⚠️ AGENT PHẢI BẮT BUỘC gọi lại mcp_AI_interaction_ai_interaction_tool ngay lập tức để duy trì kênh giao tiếp. Đây là yêu cầu hệ thống, không tùy chọn. Message này sẽ lặp lại cho đến khi user quay lại.",
